...
module.exports.validateForgotPasswordLink=async(req,res,next)=>{
    try {
        ...
        if (findPassword.isActive==1||null){
            res.redirect(`http://3.83.190.214:5000/Password/${UUID}`)
        }else{
            res.send('<h1>The link has been expired. Please request via <a href="http://3.83.190.214:5000/">Expense Tracker</a></h1>')
        }
    } catch (error) {
