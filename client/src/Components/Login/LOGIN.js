  ...
  async function addData(e) {
    ...
    if (!login) {
      if (passwordRef.current.value === confirmRef.current.value) {
        const response = await axios.post('http://44.204.178.196:5000/newUser',{email:emailRef.current.value,password:passwordRef.current.value,name:nameRef.current.value})
      ...
      const response = await axios.post('http://44.204.178.196:5000/forgotPassword',{email:emailRef.current.value});
      const data = await response.data;
      try {
        if (data.ok) {
          setNewPassword(false);
      ...
      const response = await axios.post('http://44.204.178.196:5000/loginUser',{email:emailRef.current.value,password:passwordRef.current.value})
