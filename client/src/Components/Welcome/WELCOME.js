...
const WELCOME = () => {
  ...
  async function activation(e) {
    ...
    if (!activatePremium) {
      const response = await axios.get(
        "https://3.83.190.214:5000/purchasePremium",
        ...
        var options = {
          ...
          handler: async (response) => {
            const response2 = await axios.post(
              "https://3.83.190.214:5000/updateTransactionStatus",
              {
  ...
  async function downloadExpenses(e) {
    setMenu(false)
    const response = await axios.get("https://3.83.190.214:5000/downloadAWS", {
      headers: { Authorization: idToken },
    });
    const data = await response.data;
    try {
